#!/bin/bash -xv
version="0.0.56"

docker build -t adresbook .

docker tag adresbook dockerpinguin/adresbook:latest
docker tag adresbook dockerpinguin/adresbook:$version

docker push dockerpinguin/adresbook:latest
docker push dockerpinguin/adresbook:$version 


# Assuming your local IP is 192.168.1.X or you use host.docker.internal for Mac/Windows Docker Desktop
# On Linux, you might need to find your bridge IP or use --network host for simple local testing
docker0=`ip -4 addr show docker0 | grep -oP '(?<=inet\s)\d+(\.\d+){3}'`

echo "To run the docker container againt a local API use:"
echo docker run -d -p 80:80 \
  -e API_BASE_URL="http://${docker0}:8080/adres/api/v1" \
  -e ADRES_APP_PROXY_URL="http://${docker0}:8080" \
  --network host \
  adresbook
